  ...
  try {
    ...
    try {
      ...
      if (
        ...
        session = await createNewTwitterSession(url);
        const retryUpdatedQueryParams = updateTweetIdInQueryParams(
          session.query_params as string,
          tweetId
        );
        const retryApiUrl = `${session.url}?${retryUpdatedQueryParams}`;
        data = await makeTwitterApiRequest(retryApiUrl, session.headers);
      } else if (error.message.includes('400')) {
        return new Response(
          JSON.stringify({
            error:
              'This content is not visible. It may be restricted or sensitive.',
          }),
    ...
    // Extract tweet data
    const tweetData = extractTweetData(data);
