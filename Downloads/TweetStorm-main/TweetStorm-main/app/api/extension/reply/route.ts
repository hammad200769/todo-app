...
import {
  formatKeywordsArray,
  getMaxTokens,
  getSystemPromptForTweetReplyGeneration,
  jsonErrorResponse,
...
const handlePost: RouteHandler = async (request, next) => {
  try {
    ...
    const { error } =
      validationSchemas.tweetReplyGenerationExtension.validate(data);
    if (error) {
    ...
    const tweetPrompt = getTweetReplyPrompt({
    ...
    const systemPrompt = getSystemPromptForTweetReplyGeneration(
    ...
    await db('user_generations').insert({
      ...
      type: USER_GENERATION_TYPE_TO_NUMBER_MAP.ReplyTweet,
