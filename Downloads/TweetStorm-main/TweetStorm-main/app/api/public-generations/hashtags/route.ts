...
import {
  getSystemPromptForHashtagsGeneration,
  jsonErrorResponse,
  openaiClient,
...
const handlePost: RouteHandler = async (request, next) => {
  try {
    ...
    const totalGenerations = await getTotalPublicGenerations(
      clientIp,
      USER_GENERATION_TYPE_TO_NUMBER_MAP.Hashtags
    );
    if (totalGenerations >= MAX_PUBLIC_GENERATIONS_PER_TYPE) {
      return jsonErrorResponse(
        'limit-reached',
        `You can only generate hashtags ${MAX_PUBLIC_GENERATIONS_PER_TYPE} times when logged out. Login to generate more.`,
        403
      );
    }
    ...
    const chatCompletion = await openaiClient.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: getSystemPromptForHashtagsGeneration(),
        },
        {
          role: 'user',
    ...
    await db('public_generations').insert({
      type: USER_GENERATION_TYPE_TO_NUMBER_MAP.Hashtags,
      ip_address: clientIp,
      created_at: new Date(),
    });
